...
namespace Nikon
{
    ...
    internal delegate void StartCapabilityDelegate(eNkMAIDCapability cap, eNkMAIDDataType type, Int32 data);
    #endregion
    ...
    public class NikonDevice : NikonBase
    {
        ...
        void HandleAddPreviewImage(uint data)
        {
            try
            {
                // Note:
                ...
                {
                    Object.SetUnsigned(eNkMAIDCapability.kNkMAIDCapability_CurrentPreviewID, data);
                }
        ...
        void HandleAddChild(uint id)
        {
            NikonObject item = new NikonObject(Md3, Object, id);

            List<uint> dataIds = new List<uint>();

            item.Open();
            item.Event += (NikonObject obj, IntPtr refClient, eNkMAIDEvent currentEvent, uint data) =>
            {
